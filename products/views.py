...
# Check access with auth login decorator
...
def all_products(request):
    ...
    # Set query to None
    query = None
    # For search queries, 'q' is the name of the query form set in base.html
    if request.GET:
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
    ...
    # Define in which file the 'context' will be rendered in
...
def add_product(request):
    ...
    # Limit user access, if not superuser
    ...
    if request.method == 'POST':
        ...
        # Render empty instance of the form with type and format set to none
        form = ProductForm(
            initial={
                'type': 0,
                'format': 0,
...
def add_custom_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # if current category is '1' (in the DB 1 = custom_product)
...
def edit_product(request, product_id):
    ...
    if request.user.is_authenticated and \
      str(product.category) == 'custom_product' or request.user.is_superuser:
        # If post
        if request.method == 'POST':
            # Make instance of product form with
            # request product and request files
